...
describe('computeOutcomes', () => {
  ...
  it('quantifies retry reduction when adoption lowers retries', () => {
    ...
    expect(r.estTokensSaved).toBe(4400); // 4 * (1000 input + 100 output) full turn
    expect(r.netTokensSaved).toBe(4400); // no tool spend passed
  });
  ...
  it('subtracts the tool spend to report NET savings', () => {
    ...
    expect(r.netTokensSaved).toBe(4400 - 1500);
  });
